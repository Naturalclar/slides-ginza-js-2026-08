...
export const sections: Section[] = [
  ...
  },
  {
    ...
    note: "3:20–4:55 技術の芯その1、95秒。【削る順②は消化済み】圧縮版なので結論から言う: 判断はLLM、書き換えは決定的で冪等なスクリプト。LLMに直接ファイルを触らせない——この分担がシステムを壊さないコツ。具体を1つだけ添える: ノートへの書き込みはDeno + TypeScriptのMCPサーバがやっている。外部データとの同期の詳細には踏み込まない。さらに押されたら結論の1文だけ残す",
  },
  {
    id: "failures",
    content: <h1>失敗した話を3つ</h1>,
    note: "4:55–7:25 技術の芯その2、絶対に削らない。①祝日を「昨日は何もしていない」と報告した（休みの日は空の日ではない）②曲名の部分一致で別の曲に当てた（「怪獣」と「怪獣の花唄」）③重複チケットを「完了」で消そうとした（重複は終わった仕事ではない、行ごと消すのが正しい）。どれも運用して初めて出た仕様バグ",
  },
  {
    id: "two-weeks",
