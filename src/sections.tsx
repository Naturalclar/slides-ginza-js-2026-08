...
`}</pre>
  ...
  },
  {
    ...
    note: "4:55–5:35 当初の問題。日誌に「これ直したい」と書くのはこのリポジトリ、実際に手を動かすのは別のリポジトリ。渡すたびに、背景・今どうなっているか・何をしてほしいかを手で説明し直していた。しかも日誌の記述は古くなっていることがあるので、そのまま渡すと嘘を渡すことになる。40秒",
  },
  {
    ...
    note: "5:35–6:20 解決。Claude Codeにセッション間メッセージングが入って楽になった。handoffスキルの流れ: ①渡す先のリポジトリを実際に調べる（日誌の思い込みではなく現状を確認）②背景・現状・Gap分析・やること・検証手順を書いた自己完結のHANDOFF-*.mdを置く ③そのリポジトリで動いているセッションを見つけてSendMessageで知らせる ④向こうが終わると返信がこちらに戻る。45秒",
  },
  {
    ...
    note: "6:20–6:55 含意。セッション間で渡るのはプレーンテキスト1通だけで、会話履歴もファイルも権限も一緒には行かない。だから「調べて自己完結の文書を書く」工程が必要になる——人間同士の引き継ぎと同じで、口頭の勢いは渡らない。ここが一番の学びだったと言って次へ。35秒",
  },
  {
    id: "two-weeks",
    content: <h1>2週間で、朝の「今日何やるんだっけ」が消えた</h1>,
    note: "6:55–7:25 後日談、30秒。毎朝の立ち上がりの摩擦がなくなった。スキルは足すたびに翌日から使える——作って終わりではなく、毎日使うものだけが残った。【削る順①】押されたらここを落とす。芯ではないが、運用が続いている証拠なので優先度は高い",
  },
  {
    id: "spinoff",
    content: (
  ...
  },
  {
    ...
    note: "9:00–10:00。この1枚を出しっぱなしにして、質問は口頭で募る（「何かありますか？」）。リンクが映ったままなので、質疑の間ずっと参照できる。ブログに詳細記事を書く予定と一言。想定質問: ノートが壊れたら？（全部Gitなので戻せる）／スキルってどう書く？（Markdownの手順書、コードじゃない）／会社のタスク管理とどう繋いでる？（読み取り同期のみ・書き込みは慎重に）／料金は？（実際の使用感で答える）。社名・チケットID・リスナー名・実タスクの内容は出さない",
  },
];
