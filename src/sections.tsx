...
`}</pre>
  ...
  },
  {
    id: "demo",
    content: <h1>デモ</h1>,
    note: "1:50–3:20 本体。今日のノート（デモ用に用意したもの）を開いて、①朝: ブランチが切られノートが生成されPRが開く ②昼: 日記を書くとAIの提案が更新されコミットされる ③夜: 1日がPRごとmainにマージされ、1日=1コミットになる、を見せる。オフライン対策: ローカルで完結。保険: 30秒録画を別ウィンドウに。絶対に削らない",
  },
  {
    id: "structure",
    content: (
  ...
  },
  {
    ...
    note: "3:40–4:05 経路は2つある。①スマホ: Claudeのモバイルアプリからスキルを叩く。セッションは使い捨てのコンテナなので、スキル側が必ずcommitとpushまでやる——状態をセッションに置かない設計。②腰を据えるとき: iPad miniからTailscale経由でMacにSSH、Termiusで繋いでtmuxにアタッチし、Claude Codeをそのまま動かす。tmux new -A -s main なので切れても同じセッションに戻れる。ルータもファイアウォールも触っていない。20〜25秒。押されたら「状態はセッションではなくGitにある」の一言だけ残す",
  },
  {
    id: "llm-and-scripts",
    content: (
    ...
    note: "4:05–5:20 技術の芯その1、75秒。【削る順②は消化済み】圧縮版なので結論から言う: 判断はLLM、書き換えは決定的で冪等なスクリプト。LLMに直接ファイルを触らせない——この分担がシステムを壊さないコツ。具体はスライドに出ているMCPサーバの1点だけ。Ginza.jsなので「書き換え側はTypeScript」は拾ってもらえるはず。外部データとの同期の詳細には踏み込まない。さらに押されたら結論の1文だけ残す",
  },
  {
    id: "handoff-problem",
    content: (
    ...
    note: "5:20–6:00 当初の問題。日誌に「これ直したい」と書くのはこのリポジトリ、実際に手を動かすのは別のリポジトリ。渡すたびに、背景・今どうなっているか・何をしてほしいかを手で説明し直していた。しかも日誌の記述は古くなっていることがあるので、そのまま渡すと嘘を渡すことになる。40秒",
  },
  {
    ...
    note: "6:00–6:45 解決。Claude Codeにセッション間メッセージングが入って楽になった。handoffスキルの流れ: ①渡す先のリポジトリを実際に調べる（日誌の思い込みではなく現状を確認）②背景・現状・Gap分析・やること・検証手順を書いた自己完結のHANDOFF-*.mdを置く ③そのリポジトリで動いているセッションを見つけてSendMessageで知らせる ④向こうが終わると返信がこちらに戻る。45秒",
  },
  {
    ...
    note: "6:45–7:20 含意。セッション間で渡るのはプレーンテキスト1通だけで、会話履歴もファイルも権限も一緒には行かない。だから「調べて自己完結の文書を書く」工程が必要になる——人間同士の引き継ぎと同じで、口頭の勢いは渡らない。ここが一番の学びだったと言って次へ。35秒",
  },
  {
    id: "two-weeks",
  ...
  },
  {
    ...
    note: "7:20–7:50 一例紹介、30秒厳守。配信のセトリを自動で組むWebサービス。バニラのHTML + JSでフレームワークなし、GitHub Pages。日記に書いた思いつきが当日デプロイまで行き、今も毎日使っている。日誌の記録がそのまま初期データになった。「運用のなかで思いつきが形になる」実例として30秒で流す。押されても削らない——30秒厳守で必ず触れる",
  },
  {
    id: "operate",
    content: <h1>完成させるな、運用しろ</h1>,
    note: "7:50–9:00 まとめ。【削る順③】後半は落とす前提。AIがコードを書く時代、作ること自体は安くなった。価値が残るのは、運用して初めて分かる失敗と、それを直すループを持っていること。押されていなくても後半は落とす。さらに押されたら1行目で締める",
  },
  {
    id: "thanks",
    content: (
    ...
    note: "9:00–10:00。この1枚を出しっぱなしにして、質問は口頭で募る（「何かありますか？」）。リンクが映ったままなので、質疑の間ずっと参照できる。ブログに詳細記事を書く予定と一言。想定質問: ノートが壊れたら？（全部Gitなので戻せる）／スキルってどう書く？（Markdownの手順書、コードじゃない）／会社のタスク管理とどう繋いでる？（読み取り同期のみ・書き込みは慎重に）／料金は？（実際の使用感で答える）。会社とプロダクトの名前は自分のものなので出してよい。出さないのはチケットID・リスナー名・実際のタスク内容",
  },
];
